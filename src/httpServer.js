  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && pathName === '/api/host') {
        ...
        const periodMs =
          period === '12h' ? 12 * 60 * 60 * 1000
          : period === '3d' ? 3 * 24 * 60 * 60 * 1000
          : 24 * 60 * 60 * 1000;
        const day = roblox.getUptimeStats(24 * 60 * 60 * 1000);
        const range = roblox.getUptimeStats(periodMs);
        const history = roblox.getHistory(periodMs);
        const maxPoints = 288;
        let samples = history;
        ...
        json(res, 200, {
          ...
          todayOnlineChecks: day.onlineChecks,
          currentStreakMs: day.currentStreakMs || 0,
          checkIntervalMs: CHECK_INTERVAL_MS,
          period,
          periodUptimePercent: Number(range.uptimePercent.toFixed(1)),
          periodChecks: range.totalChecks,
          periodOnlineChecks: range.onlineChecks,
          longestOnlineMs: range.longestOnlineMs || 0,
          longestOfflineMs: range.longestOfflineMs || 0,
          samples: samples.map(e => ({ t: e.ts, on: e.online ? 1 : 0 })),
          sessions: (range.sessions || []).slice(-24).map(s => ({
            online: !!s.online,
            start: s.start,
            end: s.end,
          })),
