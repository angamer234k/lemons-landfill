  ...
  try {
    ...
    switch (name) {
      ...
      case 'customize_embed': {
        ...
        if (newColor) {
          // Normalize color to number for consistent storage
          if (typeof newColor === 'string') {
            const hex = newColor.replace(/^#/, '');
            if (/^[0-9A-Fa-f]{6}$/.test(hex)) {
              newColor = parseInt(hex, 16);
            }
          }
        } else {
          // Fallback gradual color as hex string (setColor accepts it)
          newColor = getGradualColor(thread?.replies || 0);
        }

        // Persist into conversation thread so future renders (status/stream/final) keep it
        if (thread) {
