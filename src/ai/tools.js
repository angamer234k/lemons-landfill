...
function getToolsForUser(isOwner) {
  ...
  const ownerTools = [
    ...
    },
    {
      ...
      function: {
        name: 'create_invite',
        description: 'Create a new lemon.space invite link.',
        parameters: {
          type: 'object',
    ...
    },
    {
      ...
      function: {
        name: 'list_invites',
        description: 'List all active lemon.space invites.',
        parameters: { type: 'object', properties: {} },
      },
    },
    {
...
function getGradualColor(replyCount) {
  const colors = ['#0099ff', '#ff6600', '#ff00ff', '#00ff00', '#ff0000', '#ffff00'];
  ...
  try {
    ...
    switch (name) {
      ...
      case 'ascii_art': {
        ...
        return { ok: true, result: generateAsciiArt(text) };
      }
      case 'random_quote':
        return { ok: true, result: getRandomQuote() };
      case 'text_encryption': {
        const text = String(args.text || '').trim();
        const shift = Math.min(25, Math.max(1, args.shift || 1));
        const action = String(args.action || 'encrypt').toLowerCase();
        if (!text) return { ok: false, error: 'Text is required' };
        return { ok: true, result: { text: caesarCipher(text, shift, action), action } };
      }
      case 'mad_libs': {
        ...
        return { ok: true, result: generateMadLib(noun, verb, adjective, adverb) };
      }
      case 'count_words': {
        const text = String(args.text || '').trim();
        if (!text) return { ok: false, error: 'Text is required' };
        return { ok: true, result: countTextStats(text) };
      }
      case 'acronym_generator': {
        const phrase = String(args.phrase || '').trim();
        if (!phrase) return { ok: false, error: 'Phrase is required' };
        return { ok: true, result: generateAcronym(phrase) };
      }
      case 'customize_embed': {
        ...
        let newTitle = args.title ? String(args.title).slice(0, 256) : (thread?.title || 'Conversation');
        let newColor = args.color;
        if (newColor) {
          if (typeof newColor === 'string') {
            const hex = newColor.replace(/^#/, '');
            if (/^[0-9A-Fa-f]{6}$/.test(hex)) newColor = parseInt(hex, 16);
          }
        } else {
          newColor = getGradualColor(thread?.replies || 0);
        }
        if (thread) {
          if (args.title) thread.title = newTitle;
          thread.embedColor = typeof newColor === 'number' ? newColor : newColor;
        }
        try {
          const embeds = aiMessage.embeds;
          if (embeds?.[0]) {
            const newEmbed = EmbedBuilder.from(embeds[0]).setTitle(newTitle).setColor(newColor);
            await safeEditMessage(aiMessage, { embeds: [newEmbed] });
          } else {
            const newEmbed = new EmbedBuilder().setTitle(newTitle).setColor(newColor);
            await safeEditMessage(aiMessage, { embeds: [newEmbed] });
          }
          const colorStr = typeof newColor === 'number' ? '#' + newColor.toString(16).padStart(6, '0') : String(newColor);
          return { ok: true, result: `Embed customized with title: ${newTitle} and color: ${colorStr}` };
        } catch (e) {
          return { ok: false, error: e.message };
        }
      }
      case 'set_ai_model': {
        ...
        return { ok: true, result: `AI model set to ${args.model}` };
      }
      case 'set_max_replies': {
        if (!isOwner) return { ok: false, error: 'Owner only' };
        const limit = Math.min(50, Math.max(1, Number(args.limit) || 5));
        botConfig.maxReplies = limit;
        return { ok: true, result: `Max replies set to ${limit}` };
      }
      case 'set_allow_others': {
        if (!isOwner) return { ok: false, error: 'Owner only' };
        botConfig.allowOthersToReply = !!args.allow;
        return { ok: true, result: `Allow others: ${botConfig.allowOthersToReply}` };
      }
      case 'view_config': {
        if (!isOwner) return { ok: false, error: 'Owner only' };
        return {
      ...
      case 'list_invites':
      case 'revoke_invite':
        return { ok: false, error: 'Invite tools not wired in this restore; use /invite command.' };
      default:
...
function getRandomQuote() {
  const quotes = [
    ...
    'Lemon first, questions later.',
  ];
  return quotes[Math.floor(Math.random() * quotes.length)];
}

function caesarCipher(text, shift, action) {
  const s = action === 'decrypt' ? (26 - (shift % 26)) % 26 : shift % 26;
...
function generateMadLib(noun, verb, adjective, adverb) {
  return `Once upon a time, a ${adjective} ${noun} decided to ${verb} ${adverb}. The end.`;
}
